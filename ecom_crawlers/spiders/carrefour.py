...
class CarreFourSpider(Spider):
    ...
    def start_requests(self):
        for main_category, subcategories in self.categories.items():
            for subcategory, category_code in subcategories:
                catalogue_code = self.get_catalogue_code(main_category)
                if catalogue_code:
                    yield scrapy.Request(
                        url=self.products_api.format(category_code, self.page),
    ...
    def parse(self, response):
        ...
        if products:
            for product in products:
                ...
                item['URL'] = self.main_url + item['URL']
    ...
    def parse_description(self, response):
        ...
        about_list = response.xpath('//div[@class="css-1npift7"]//li/text()').getall()
        about = "\n".join(about_list)
        internal_memory = response.xpath('//div[@data-testid="nonFoodItems" and not(ancestor::div[contains(@class, "css-oh2l30")])]//div[contains(text(),"Internal storage capacity")]/following-sibling::h3/text()').get()
        model_name = response.xpath('//div[@data-testid="nonFoodItems" and not(ancestor::div[contains(@class, "css-oh2l30")])]//div[contains(text(),"Model Name")]/following-sibling::h3/text()').get()
        if not model_name:
            model_name = response.xpath('//li[contains(text(), "Model name")]/text()').get()
        processor_number = response.xpath('//div[@data-testid="nonFoodItems" and not(ancestor::div[contains(@class, "css-oh2l30")])]//div[contains(text(),"Processor model")]/following-sibling::h3/text()').get()
        os_version = response.xpath('//div[@data-testid="nonFoodItems" and not(ancestor::div[contains(@class, "css-oh2l30")])]//div[contains(text(),"Operating system version")]/following-sibling::h3/text()').get()
        color = response.xpath('//div[@data-testid="nonFoodItems" and not(ancestor::div[contains(@class, "css-oh2l30")])]//div[contains(text(),"Color")]/following-sibling::h3/text()').get()
        ram = response.xpath('//div[@data-testid="nonFoodItems" and not(ancestor::div[contains(@class, "css-oh2l30")])]//div[contains(text(),"Internal RAM")]/following-sibling::h3/text()').get()
